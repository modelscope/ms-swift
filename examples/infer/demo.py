...
def infer_batch(engine: 'InferEngine', infer_requests: List['InferRequest']):
    request_config = RequestConfig(max_tokens=512, temperature=1.0, n=2)
    metric = InferStats()
    resp_list = engine.infer(infer_requests, request_config, metrics=[metric])
    query0 = infer_requests[0].messages[0]['content']
    print(f'query0: {query0}')
    print(f'response0.choice0: {resp_list[0].choices[0].message.content}')
    print(f'response0.choice1: {resp_list[0].choices[1].message.content}')
    print(f'num return choices: {len(resp_list[0].choices)}')
