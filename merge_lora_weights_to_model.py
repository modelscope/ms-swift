...
def convert_ckpt(args):
    assert args.model_type is not None and args.ckpt_dir is not None
    if not os.path.exists(args.model_type):
        args.model_type = snapshot_download(args.model_type)
    ...
    model.save_pretrained(args.ckpt_dir + '-merged')
