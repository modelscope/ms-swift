...
def infonce_loss(outputs, labels, loss_scale=None, num_items_in_batch=None) -> torch.Tensor:
    ...
    if not use_batch:
        ...
        def mask_fake_negative(sim_matrix, sim_labels):
            """
            sim_matrix: [B, N]
            sim_labels: [B]，每一行正例所在列 index
            """
            print("[DEBUG] using CUSTOM infonce_loss with FAKE_NEG_DUP_EPS logic", flush=True)

            device = sim_matrix.device
            row_idx = torch.arange(sim_matrix.size(0), device=device)

            # 每行正例分数 s_i^+
            pos_scores = sim_matrix[row_idx, sim_labels].view(-1, 1).detach()

            # 1）原本逻辑：比 (pos + 0.1) 还大的负例，当成假负例
            thresholds = (pos_scores + 0.1)
            thresholds = thresholds.detach()
            hard_mask = sim_matrix > thresholds

            # 2）新增逻辑：与正例几乎一样高的负例，也当成假负例
            dup_eps = 0.008  # 你想调严/松就改这个
            diff = torch.abs(sim_matrix - pos_scores)
            dup_mask = diff < dup_eps

            # 合并两个条件：hard 假负 + 近似重复假负
            mask = hard_mask | dup_mask

            # 一定要保留正例本身，不要被 mask
            mask[row_idx, sim_labels] = False

            # 对所有假负例位置置 -inf，让它们在 softmax 里直接消失
            sim_matrix[mask] = float('-inf')
...
class RouteClsHeads(nn.Module):

    def __init__(self, hidden_size: int, num_lv1: int, num_lv2: int, num_lv3: int):
        super().__init__()
        self.lv1_head = nn.Linear(hidden_size, num_lv1)
        self.lv2_head = nn.Linear(hidden_size, num_lv2)
        self.lv3_head = nn.Linear(hidden_size, num_lv3)

    def forward(self, features: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        return self.lv1_head(features), self.lv2_head(features), self.lv3_head(features)


class RouteHybridInfonceLoss(BaseLoss):
    """Hybrid loss for route retrieval:
    InfoNCE + weighted hierarchical CE (+ optional conditional lv3 CE and path KL).
    """

    _MODES = {'ce3', 'ce3_conditional', 'ce3_conditional_pathkl'}

    def __init__(self, args, trainer):
        super().__init__(args, trainer)
        self.mode = os.environ.get('ROUTE_CLS_MODE', 'ce3').lower()
        if self.mode not in self._MODES:
            raise ValueError(f'Unsupported ROUTE_CLS_MODE: {self.mode}, expected one of {sorted(self._MODES)}')

        self.feature_key = os.environ.get('ROUTE_CLS_FEATURE_KEY', 'pre_norm_last_hidden_state')
        self.label_smoothing = float(os.environ.get('ROUTE_CLS_LABEL_SMOOTHING', '0.05'))
        self.missing_label_strategy = os.environ.get('ROUTE_CLS_MISSING_LABEL_STRATEGY', 'error').lower()
        if self.missing_label_strategy not in {'error', 'skip'}:
            raise ValueError("ROUTE_CLS_MISSING_LABEL_STRATEGY must be one of {'error', 'skip'}")

        self._missing_label_warned = False
        self._fallback_feature_warned = False

        self.num_lv1 = int(os.environ.get('ROUTE_NUM_LV1', '2'))
        self.num_lv2 = int(os.environ.get('ROUTE_NUM_LV2', '4'))
        self.num_lv3 = int(os.environ.get('ROUTE_NUM_LV3', '200'))
        self.lv2_to_lv1 = None
        self.lv3_to_lv2 = None
        self._lv2_to_lv3_indices = None
        self._lv3_local_idx = None

        self._load_hierarchy()
        self.route_lambda, self.alpha, self.beta_pathkl = self._resolve_loss_weights()

    def _resolve_loss_weights(self) -> Tuple[float, Tuple[float, float, float], float]:
        defaults = {
            'ce3': (0.10, (0.2, 1.0, 0.3), 0.0),
            'ce3_conditional': (0.15, (0.2, 1.0, 0.5), 0.0),
            'ce3_conditional_pathkl': (0.15, (0.2, 1.0, 0.5), 0.05),
        }
        default_lambda, default_alpha, default_beta = defaults[self.mode]

        route_lambda = float(os.environ.get('ROUTE_CLS_LAMBDA', str(default_lambda)))
        alpha_raw = os.environ.get('ROUTE_CLS_ALPHA', None)
        if alpha_raw is None:
            alpha = default_alpha
        else:
            alpha_items = [float(x.strip()) for x in alpha_raw.split(',') if x.strip()]
            if len(alpha_items) != 3:
                raise ValueError(f'ROUTE_CLS_ALPHA must contain 3 comma-separated values, got: {alpha_raw}')
            alpha = (alpha_items[0], alpha_items[1], alpha_items[2])
        beta = float(os.environ.get('ROUTE_CLS_BETA_PATHKL', str(default_beta)))
        return route_lambda, alpha, beta

    def _load_hierarchy(self) -> None:
        hierarchy_path = os.environ.get('ROUTE_HIERARCHY_JSON', None)
        if hierarchy_path:
            with open(hierarchy_path, 'r', encoding='utf-8') as f:
                hierarchy = json.load(f)
            self.num_lv1 = int(hierarchy.get('num_lv1', self.num_lv1))
            self.num_lv2 = int(hierarchy.get('num_lv2', self.num_lv2))
            self.num_lv3 = int(hierarchy.get('num_lv3', self.num_lv3))
            self.lv2_to_lv1 = hierarchy.get('lv2_to_lv1')
            self.lv3_to_lv2 = hierarchy.get('lv3_to_lv2')
            if self.lv2_to_lv1 is not None:
                self.lv2_to_lv1 = [int(x) for x in self.lv2_to_lv1]
            if self.lv3_to_lv2 is not None:
                self.lv3_to_lv2 = [int(x) for x in self.lv3_to_lv2]

        if self.mode in {'ce3_conditional', 'ce3_conditional_pathkl'}:
            if self.lv3_to_lv2 is None:
                raise ValueError(
                    'ROUTE_HIERARCHY_JSON with lv3_to_lv2 is required for conditional/path-kl modes.')
            if len(self.lv3_to_lv2) != self.num_lv3:
                raise ValueError(f'len(lv3_to_lv2) must equal num_lv3 ({self.num_lv3}), got {len(self.lv3_to_lv2)}')
            self._build_hierarchy_indices()

    def _build_hierarchy_indices(self) -> None:
        lv2_to_lv3: Dict[int, List[int]] = {i: [] for i in range(self.num_lv2)}
        for lv3_idx, lv2_idx in enumerate(self.lv3_to_lv2):
            if lv2_idx < 0 or lv2_idx >= self.num_lv2:
                raise ValueError(f'lv3_to_lv2 contains out-of-range lv2 index: {lv2_idx}')
            lv2_to_lv3[lv2_idx].append(lv3_idx)

        for lv2_idx, children in lv2_to_lv3.items():
            if len(children) == 0:
                raise ValueError(f'lv2={lv2_idx} has no lv3 children in hierarchy mapping.')

        lv3_local_idx = {}
        for lv2_idx, children in lv2_to_lv3.items():
            lv3_local_idx[lv2_idx] = {global_idx: local_idx for local_idx, global_idx in enumerate(children)}

        self._lv2_to_lv3_indices = lv2_to_lv3
        self._lv3_local_idx = lv3_local_idx
    ...
    def _infer_hidden_size(model) -> int:
        ...
        candidates = [
            ...
            getattr(getattr(config, 'model_config', None), 'hidden_size', None),
        ]
        for c in candidates:
            if isinstance(c, int):
                return c
        ...
        raise ValueError('Unable to infer hidden_size for route classification heads.')

    def initialize_for_model(self, model) -> None:
        if model is None or hasattr(model, 'route_cls_heads'):
            return
        hidden_size = self._infer_hidden_size(model)
        heads = RouteClsHeads(hidden_size, self.num_lv1, self.num_lv2, self.num_lv3)
        ...
        for p in model.parameters():
            if p.is_floating_point():
                ...
                break
        heads.to(device=target_device, dtype=target_dtype)
        model.route_cls_heads = heads

    def _get_heads(self, model):
        if model is None:
            return None
        if hasattr(model, 'route_cls_heads'):
            return model.route_cls_heads
        if hasattr(model, 'module') and hasattr(model.module, 'route_cls_heads'):
            return model.module.route_cls_heads
        return None

    def _get_feature_tensor(self, outputs) -> torch.Tensor:
        if self.feature_key in outputs:
            features = outputs[self.feature_key]
        elif 'last_hidden_state' in outputs:
            if not self._fallback_feature_warned:
                print(
                    f'[WARN] ROUTE_CLS_FEATURE_KEY="{self.feature_key}" not found, fallback to "last_hidden_state".',
                    flush=True)
                self._fallback_feature_warned = True
            features = outputs['last_hidden_state']
        else:
            raise ValueError(
                f'Neither ROUTE_CLS_FEATURE_KEY="{self.feature_key}" nor "last_hidden_state" exists in outputs.')
        if features.dim() == 3:
            features = features[:, 0]
        return features

    def _extract_query_features(self, features: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        # split into [anchor, positive, negatives...], and use anchor as query feature
        split_tensors = _parse_multi_negative_sentences(features, labels, hard_negatives=None)
        if len(split_tensors) == 0:
            raise ValueError('No valid embedding groups found for route classification.')
        return torch.stack([tensor[0] for tensor in split_tensors], dim=0)

    @staticmethod
    def _as_label_tensor(value, name: str, device: torch.device, expected_size: int) -> Optional[torch.Tensor]:
        if value is None:
            return None
        ...
        if tensor.numel() != expected_size:
            raise ValueError(f'{name} size mismatch: expected {expected_size}, got {tensor.numel()}')
        return tensor

    def _prepare_route_labels(self, query_features: torch.Tensor, kwargs) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor], Optional[torch.Tensor]]:
        expected_size = query_features.size(0)
        device = query_features.device
        lv1 = self._as_label_tensor(kwargs.get('label_lv1'), 'label_lv1', device, expected_size)
        lv2 = self._as_label_tensor(kwargs.get('label_lv2'), 'label_lv2', device, expected_size)
        lv3 = self._as_label_tensor(kwargs.get('label_lv3'), 'label_lv3', device, expected_size)
        return lv1, lv2, lv3

    def _handle_missing_labels(self, infonce: torch.Tensor) -> torch.Tensor:
        if self.missing_label_strategy == 'skip':
            if not self._missing_label_warned:
                print('[WARN] label_lv1/label_lv2/label_lv3 missing, skip route classification branch.', flush=True)
                self._missing_label_warned = True
            return infonce
        raise ValueError('label_lv1/label_lv2/label_lv3 are required for route_hybrid_infonce loss.')

    def _compute_lv3_conditional_ce(self, logits_lv3: torch.Tensor, label_lv2: torch.Tensor,
                                    label_lv3: torch.Tensor) -> torch.Tensor:
        losses = []
        for i in range(logits_lv3.size(0)):
            parent = int(label_lv2[i].item())
            target = int(label_lv3[i].item())
            children = self._lv2_to_lv3_indices.get(parent, None)
            local_idx_map = self._lv3_local_idx.get(parent, None)
            if children is None or local_idx_map is None:
                raise ValueError(f'Invalid lv2 label for conditional CE: {parent}')
            if target not in local_idx_map:
                raise ValueError(f'lv3 label {target} does not belong to lv2 label {parent} in hierarchy mapping.')
            local_target = torch.tensor([local_idx_map[target]], dtype=torch.long, device=logits_lv3.device)
            local_logits = logits_lv3[i, children].unsqueeze(0)
            losses.append(F.cross_entropy(local_logits, local_target, label_smoothing=self.label_smoothing))
        return torch.stack(losses).mean() if losses else torch.tensor(0.0, device=logits_lv3.device)

    def _compute_path_kl(self, logits_lv1: torch.Tensor, logits_lv2: torch.Tensor, logits_lv3: torch.Tensor,
                         label_lv1: torch.Tensor, label_lv2: torch.Tensor, label_lv3: torch.Tensor) -> torch.Tensor:
        p1 = torch.softmax(logits_lv1, dim=-1)
        p2 = torch.softmax(logits_lv2, dim=-1)

        if self.mode in {'ce3_conditional', 'ce3_conditional_pathkl'}:
            p3 = torch.zeros_like(logits_lv3)
            for i in range(logits_lv3.size(0)):
                parent = int(label_lv2[i].item())
                children = self._lv2_to_lv3_indices[parent]
                local_probs = torch.softmax(logits_lv3[i, children], dim=-1)
                p3[i, children] = local_probs
        else:
            p3 = torch.softmax(logits_lv3, dim=-1)

        pred = torch.cat([p1, p2, p3], dim=-1) / 3.0
        target = torch.zeros_like(pred)
        target[torch.arange(pred.size(0), device=pred.device), label_lv1] = 1.0
        target[torch.arange(pred.size(0), device=pred.device), self.num_lv1 + label_lv2] = 1.0
        target[torch.arange(pred.size(0), device=pred.device), self.num_lv1 + self.num_lv2 + label_lv3] = 1.0
        target = target / 3.0
        return F.kl_div(torch.log(pred.clamp_min(1e-12)), target, reduction='batchmean')

    def __call__(self, outputs, labels, **kwargs) -> torch.Tensor:
        infonce = infonce_loss(outputs, labels, kwargs.get('loss_scale'), kwargs.get('num_items_in_batch'))
        features = self._get_feature_tensor(outputs)
        query_features = self._extract_query_features(features, labels)

        label_lv1, label_lv2, label_lv3 = self._prepare_route_labels(query_features, kwargs)
        if label_lv1 is None or label_lv2 is None or label_lv3 is None:
            return self._handle_missing_labels(infonce)

        # Make sure heads are created before first optimization step.
        heads = self._get_heads(self.trainer.model)
        if heads is None:
            self.initialize_for_model(self.trainer.model)
            heads = self._get_heads(self.trainer.model)
        if heads is None:
            self.initialize_for_model(kwargs.get('trainer').model if kwargs.get('trainer') is not None else None)
            heads = self._get_heads(self.trainer.model)
        if heads is None:
            raise RuntimeError('route_cls_heads is not initialized on model.')
        if next(heads.parameters()).device != query_features.device:
            heads.to(query_features.device)
        head_dtype = next(heads.parameters()).dtype
        if query_features.dtype != head_dtype:
            query_features = query_features.to(head_dtype)
        logits_lv1, logits_lv2, logits_lv3 = heads(query_features)

        ce_lv1 = F.cross_entropy(logits_lv1, label_lv1, label_smoothing=self.label_smoothing)
        ce_lv2 = F.cross_entropy(logits_lv2, label_lv2, label_smoothing=self.label_smoothing)
        if self.mode in {'ce3_conditional', 'ce3_conditional_pathkl'}:
            ce_lv3 = self._compute_lv3_conditional_ce(logits_lv3, label_lv2, label_lv3)
        else:
            ce_lv3 = F.cross_entropy(logits_lv3, label_lv3, label_smoothing=self.label_smoothing)

        cls_loss = self.route_lambda * (self.alpha[0] * ce_lv1 + self.alpha[1] * ce_lv2 + self.alpha[2] * ce_lv3)
        total_loss = infonce + cls_loss

        if self.mode == 'ce3_conditional_pathkl':
            path_kl = self._compute_path_kl(logits_lv1, logits_lv2, logits_lv3, label_lv1, label_lv2, label_lv3)
            total_loss = total_loss + self.beta_pathkl * path_kl
