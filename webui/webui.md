...
1. **AI 助手**（上文完全没有这一层）：编排页「流程助手」+ 节点上的「问 AI」；训练/评测/导出/部署四个新建页的「配置助手」，能读当前表单字段、做配置体检（如导出「要量化但没合并 LoRA」）。铁律是 AI 只提议，每条改动是一张卡片，点「应用」才落到图/表单上
2. **偏好设置页** `/settings`：tips 开关与轮换间隔、AI 入口开关、AI 改动是否需确认、节点跑完自动展开输出；主题切换占位未接
3. **功能开关** `config/features.ts`：modules / multiTenant / chatCompare / pauseResume / workflowCodeSync，即上文 5.B 的前端落点（后续改为启动时从 `/api/features` 拉取）
4. **桌面式外壳**：深色标题栏（侧栏折叠、前进后退、文档/GitHub）、侧栏（模块导航 + 最近任务 + 空间切换 + 搜索⌘K/通知占位）、底部 tips 条
5. **列表页**：关键词搜索（名称/id/模型）、带计数的状态筛选、行内操作（指标、日志、暂停/继续、停止走二次确认）、「重命名 / 以此为模板新建 / 删除」菜单
6. **详情页 Tab 走 URL 段**（`/train/:id/metrics`），刷新和分享链接都停在同一个 Tab
7. **新建页右侧实时命令预览**；底部两个出口「存为草稿 / 提交运行」
8. **编排页**：图形/YAML/代码三视图、预置示例（GRPO、SFT、DPO、多轮 GRPO）、单节点运行、整图运行的暂停/继续/停止、节点输出面板
9. **部署详情**给 curl 与 Python（OpenAI 兼容）调用示例，「试用」跳对话页
10. 页面底部的滚动小 tips，属于小巧思

### 上文写了、界面未做或做法不同

1. **autoresearch / 自进化 RSI**（1.A、1.B）：无任何界面
2. **对话页的 skills / tools / prompts**（2.C、页面列表）：没有加载入口；只有联网搜索、代码、思考档位三个开关，均为占位。会话列表也不能删除对话
3. **数据集批量推理**（2.B）：无
4. **训练前后推理对比**（2.A）：只有通用的多模型并排，不区分 base/tuned，也没有「同时加载显存翻倍」的告警
5. **评测对比与历史管理**（3.A、3.B）：雷达图和分数表只在单个评测详情内对比几个 mock 模型，没有跨任务挑选对比的入口
6. **ckpt 血缘**（4.B）：只在字段说明文案里提到 lineage.json，界面上没有血缘展示
7. **展示和编辑 python 代码并开启训练**（6）：代码视图只读（可复制），另有一个「模拟手改代码」按钮用于演示不一致流程；新建页没有脚本编辑
8. **提交前显存检查告警**：无。只有评测页一条 evalscope 未安装的提示
9. **列表分页**：无，一次性渲染 + 前端过滤
10. **workflow 定义与运行实例两层**：编排列表复用任务列表，只有实例一层；编辑器只能换预置示例，没有保存/另存定义
11. **节点间引用语法** `${train-xxx.outputs.best_checkpoint}`：生成的 YAML 用 `needs:` 表达依赖，没有引用语法
12. **循环组件**：画布上是「循环框」视觉分区，落到 YAML 只是一行注释，不是真组件
13. **参数表单由 json_schema 生成**：当前是手写的 label/value 键值对
14. **从失败节点续跑**：只有整图暂停后「从没跑完的节点接上」，没有失败续跑入口
15. **训练详情无产物视图**：只有指标和日志两个 Tab，outputs / checkpoint 列表没有出口
