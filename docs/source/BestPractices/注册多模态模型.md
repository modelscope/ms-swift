...
本文将介绍如何在ms-swift中注册多模态模型，并成功推理和训练。本文将以Qwen2.5-Omni为例子，注册新的model_type和template，并支持文本、图片、视频和音频的训练。由于Qwen2.5-Omni已经在ms-swift中注册，我们可以通过显式指定model_type和template来使用我们自定义的部分。
...
swift sft \
    ...
    --model_type my_qwen2_5_omni \
    --template my_qwen2_5_omni \
    --custom_register_path 'custom.py' \
