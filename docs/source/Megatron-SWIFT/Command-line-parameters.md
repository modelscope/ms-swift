...
- log_backend: 启用的日志后端。默认为None。可选项为'wandb'和'swanlab'。（tensorboard会一直启动）。登陆可以使用`WANDB_API_KEY`、`SWANLAB_API_KEY`环境变量。
- wandb_project: wandb/swanlab 项目名称，取决于`log_backend`。默认为'megatron-swift'。
- wandb_exp_name: wandb/swanlab 实验名称。默认为`--save`的值。
- wandb_save_dir: 本地保存 wandb/swanlab 结果的路径。默认为None。
