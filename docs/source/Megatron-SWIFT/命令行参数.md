...
- 🔥overlap_grad_reduce: 启用DDP中grad reduce操作的重叠（降低DP通信耗时）。默认为True。
  - 该参数在"ms-swift>=3.9"默认值从False修改为True。
- 🔥overlap_param_gather: 启用分布式优化器中参数all-gather的重叠（降低DP通信耗时）。默认为True。
  - 该参数在"ms-swift>=3.9"默认值从False修改为True。
...
- 🔥num_virtual_stages_per_pipeline_rank: 每个流水线并行 rank 的虚拟流水线阶段数量。默认为None。vpp并行，用于减少pp并行的计算空泡，提高GPU利用率。例子参考[这里](https://github.com/modelscope/ms-swift/tree/main/examples/megatron/vpp)。
