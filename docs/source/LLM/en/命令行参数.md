...
- `--dataset`: Datasets to use for training, default is `[]`. Available datasets can be found in [Supported Datasets](Supported Models and Datasets.md#Datasets). To use multiple datasets for training, separate them with ',' or ' ', e.g. `--dataset alpaca-en,alpaca-zh` or `--dataset alpaca-en alpaca-zh`.
...
- `--custom_train_dataset_path`: Default is `[]`. See [Customization and Extension](Customization and Extension.md) for details.
- `--custom_val_dataset_path`: Default is `[]`. See [Customization and Extension](Customization and Extension.md) for details.
- `--self_cognition_sample`: Sampling number for self-cognition dataset. Default is `0`. When setting this to >0, you also need to specify `--model_name`, `--model_author`. See [Self-Cognition Fine-tuning Best Practices](Self-Cognition Fine-tuning Best Practices.md) for more info.
...
- `--use_flash_attn`: Whether to use flash attn, default is `None`. Installation steps for flash_attn can be found at [https://github.com/Dao-AILab/flash-attention](https://github.com/Dao-AILab/flash-attention). Models supporting flash_attn can be found in [LLM Supported Models](Supported Models and Datasets.md).
