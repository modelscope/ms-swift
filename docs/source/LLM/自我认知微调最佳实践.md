...
## 微调
提示:
- 因为自我认知训练涉及到知识编辑, 建议对**MLP**加lora_target_modules. 你可以通过指定`lora_target_modules='ALL'`在所有的linear层(包括qkvo以及mlp)加lora. 这**通常是效果最好的**.
...
## 微调后推理
你需要设置`best_model_checkpoint`的赋值, 该值会在sft的最后被打印出来.
...
swift infer --ckpt_dir 'qwen-7b-chat/vx-xxx/checkpoint-xxx' --eval_human true
...
# 直接使用web-ui
CUDA_VISIBLE_DEVICES=0 \
swift web-ui --ckpt_dir 'qwen-7b-chat/vx-xxx/checkpoint-xxx' --eval_human true
