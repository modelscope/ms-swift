...
- `--tools_prompt`: 选择tools字段转化的相应tools system prompt, 可选项为['default', 'toolbench'], 分别为ReAct格式和toolbench格式。如果你想了解更多，可以参考[Agent部署最佳实践](Agent部署最佳实践.md)
...
- `--tools_prompt`: 默认值为`default`. 具体的参数介绍可以在`sft.sh命令行参数`中查看.
