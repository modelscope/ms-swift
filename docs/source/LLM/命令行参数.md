...
- `--lora_target_modules`: 指定lora模块, 默认为`['DEFAULT']`. 如果lora_target_modules传入`'DEFAULT'` or `'AUTO'`, 则根据`model_type`查找`MODEL_MAPPING`中的`lora_target_modules`(默认指定为qkv). 如果传入`'ALL'`, 则将所有的Linear层(不含head)指定为lora模块. 如果传入`'ALL_WITH_EMBEDDING'`, 则将所有Linear(不含head)以及Embedding层指定为lora模块. 如果内存允许, 建议设置成'ALL' or 'ALL_WITH_EMBEDDING'. 如果传入该参数只有当`sft_type`指定为'lora'时才生效.
