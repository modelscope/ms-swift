...
- `--beta`: KL正则项系数, `simpo`算法默认为2.0, 其他算法默认为0.1, 值越大KL, 表示与参考模型偏离越小.
- `--label_smoothing`: 是否使用DPO smoothing, 默认值为0，一般设置在0~0.5之间.
- `--loss_type`: loss类型, 默认值'sigmoid'. 不同算法支持的loss类型请查看文档xxx
...
- `desirable_weight`: KTO算法中对desirable response的loss权重，默认为1.0
- `undesirable_weight`: KTO算法中对undesirable response的loss权重, 默认为1.0. 分别用nd和nu表示数据集中desirable examples和undesirable examples的数量，论文中推荐控制nd\*desirable_weight/nu\*undesirable_weight位于[1,4/3]区间，
