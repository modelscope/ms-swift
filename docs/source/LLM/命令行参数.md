...
- `--loss_scale_config_path`: `use_loss_scale`为True时生效，设置自定义的loss_scale配置，一般搭配Agent训练，扩大`Action`等ReAct字段的loss
  - 你可以以字典的形式配置loss_scale，其中键对应特殊字段, 值表示特殊字段和后续字段的loss_scale。以`"Observation:": [2, 0]`为例，比如response为`xxxx Observation:error`, 在计算loss时会将`Observation:`部分的字段loss扩大两倍，并且不计算`error`的部分。同时支持正则匹配式，键对应匹配模式，值表示匹配字段的loss_scale, 例如：`"r'<.*?>'":[1.0]`.
  - 同时支持匹配query对整段response设置loss_scale, 比如对[Agent-Flan](https://arxiv.org/abs/2403.12881)论文中涉及的固定多轮对话query，我们可以匹配其中的固定query来设置，你可以参考`swift/llm/agent/agentflan.json`
  - 默认使用`swift/llm/agent/default_loss_scale_config.json`, 对`Action:`,`Action Input:`,`Thought:`,`Final Answer:`和`Observation:`字段进行loss_scale
