...
- `--merge_lora`: 默认为`None`. 如果`sft_type`为'lora', 则设置为True, 否则设置为False. 覆盖InferArguments中的`merge_lora`参数.
- `--quant_bits`: awq量化的bits数. 默认为`0`, 即不进行量化. 你可以设置为`4`进行4bits量化.
...
- `--push_to_hub`: 默认为`False`. 是否将最后的`ckpt_dir`push到ModelScope Hub中. 如果你指定了merge_lora, 则将推送全量参数; 如果你还指定了quant_bits, 则将推送量化后的模型.
