...
- [sft 参数](#sft-参数)
- [dpo 参数](#dpo-参数)
- [export infer 参数](#export-infer-参数)
- [app-ui 参数](#app-ui-参数)
- [deploy 参数](#deploy-参数)
...
- `--merge_lora`: 是否将lora权重merge到基模型中, 并保存完整的权重, 默认为`False`. 权重会保存在`ckpt_dir`的同级目录中,  e.g. `'/path/to/your/vx_xxx/checkpoint-xxx-merged'`目录下.
