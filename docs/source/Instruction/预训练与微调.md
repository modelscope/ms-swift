...
- QLoRA训练的模型Merge LoRA会有较大误差（BNB/HQQ支持merge-lora，GPTQ/AWQ不支持），不建议使用QLoRA进行微调。建议使用LoRA/全参数进行微调，合并为完整权重后再使用GPTQ/AWQ/BNB进行[量化](https://github.com/modelscope/ms-swift/tree/main/examples/export/quantize)。
