...
- 🔥max_pixels: 多模态模型输入图片的最大像素数（HW），将超过该限制的图像进行缩放（避免训练OOM）。默认为None，不限制最大像素数。
...
reward模型参数将在PPO、GRPO中使用, teacher模型参数将在GKD中使用。
...
- vllm_mode server 参数
  ...
  - vllm_server_base_url: vLLM server的Base URL(比如 [http://local_host:8000](http://local_host:8000)), 默认为None。设置后，忽略host和port设置。
