...
- streaming_shard: 将流式训练集切分到各个数据并行rank上，每个rank只预处理属于自己的那一份，不再由rank0统一预处理并分发，默认为False。该参数只在`--streaming true`时生效。所有rank必须以相同、确定的顺序迭代原始流；内置数据集加载器会在各rank使用相同的shuffle seed。
  ...
  - 开packing时，每个rank都会真正用上自己的`--dataset_num_proc`个encode子进程（默认路径下只有rank0在用，其余rank的子进程从头空转到尾），因此单节点的CPU与内存占用可能按「本节点rank数 × dataset_num_proc」增长；若节点CPU或内存吃紧，请调小`dataset_num_proc`。不开packing时，流式encode仍是每rank单进程。
  - 切分以"连续原始样本块"为单位，块大小取dataloader每产出一个item所消耗的原始样本数（即per_device_train_batch_size；packing时为`packing_interval`），rank `r`保留第`r`、`r+world_size`、`r+2*world_size`……块。不开packing时，只有在预处理确定、且每条原始样本恰好产生一条输出时，各rank的样本顺序才与默认路径相同。样本被删除（如`truncation_strategy=delete`或坏图片）、展开（`truncation_strategy=split`）或随机增强时，编码后的rank分配可能变化。
  - 开packing时，每个rank会自行重新装包，且余量箱只在该rank内部传递，因此pack分组、pack数量与有限数据流的尾部覆盖都可能不同于dispatch路径。
  - 代价：每个rank都会完整读取一遍原始数据流，被切分的是template encode与流式packing阶段。若原始数据是"jsonl存路径 + 外部图像"这类形态，重复读取通常只是廉价的文本解析；若有昂贵的自定义数据预处理，或原始数据本身内嵌大量二进制（例如内嵌图片bytes的parquet），多机场景下需要先评估重复读取的开销。
  - 原始数据块的切分与文件个数、大小无关，各rank的原始样本数最多相差一个块。停止策略是**first exhausted**：任一rank的编码batch耗尽时所有rank同步停止。这样可避免分布式hang，但若过滤或packing导致各rank的batch数失衡，其他rank上尚未消费的数据量没有固定上界。shuffle通常会降低系统性失衡；若训练由`max_steps`提前结束、未到数据耗尽点，则不受影响。
  - 只对训练集生效，验证集仍走默认路径。
  - 不支持GRPO与GKD，因为这两类训练会把encode延迟到数据集后处理阶段之后。
