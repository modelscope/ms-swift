...
- SWIFT_ALLOW_INTERNAL_URL: 默认为`'0'`。在获取多模态媒体URL（图片/音频/视频）时，swift 会拒绝解析到非公网地址（回环、内网私有网段等）的 URL，以防御 SSRF；这对可被非信任方访问的 `swift deploy` 服务尤为重要。当媒体 URL 可信时（例如离线训练中使用的内网图片服务器），可将其设置为`'1'`以放行此类地址。云厂商元数据端点（`169.254.0.0/16`、阿里云 `100.100.100.200` 等）无论该开关如何设置都始终被拦截。
- SWIFT_URL_ALLOWED_HOSTS: 以逗号分隔的 host 白名单，例如 `bucket.oss-cn-hangzhou.aliyuncs.com,cdn.example.com`。设置后，仅当媒体 URL 的 host 在该列表中时才会被获取；列表中的 host 可以解析到普通内网地址，但云厂商元数据地址仍会被拒绝。当部署服务的媒体来自已知的存储桶/CDN 域名时推荐配置。
- SWIFT_MAX_DOWNLOAD_SIZE_MB: 获取媒体 URL 时单个响应体的大小上限，默认为`1024`（即1GB）。用于防止非信任方通过指向超大或无限响应的 URL 耗尽内存（视频链路还会写入磁盘临时文件）。设置为`0`可关闭该限制。
- SWIFT_MEDIA_ALLOWED_DIRS: 以逗号分隔的**绝对**目录白名单，例如 `/data/images,/data/videos`（写相对路径会按当前工作目录解析，通常不是你想要的）。默认不设置，此时行为不变（可从磁盘任意位置读取媒体文件，训练与本地推理依赖该行为）。设置后，媒体字符串解析出的本地文件路径必须位于这些目录之内，否则拒绝读取；路径会解析符号链接与 `..` 后再比较，因此指向白名单之外的符号链接同样会被拒绝。推荐在 `swift deploy` 暴露给非信任方时配置：非信任方本应只传 URL 或 base64，若传入本地路径，服务端会读取该文件并交给模型描述其内容，等于可读取服务器上的任意文件。若要完全禁止本地路径，将其指向一个不含其他文件的空目录即可。注意：训练与本地推理默认无需设置该变量；若在训练环境中也设置了它（例如训练与部署共用同一环境），需将训练数据所在目录一并加入白名单，否则训练读取白名单外的本地媒体文件会被拒绝。
