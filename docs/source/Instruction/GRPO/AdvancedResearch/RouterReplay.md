...
| 场景 | 行为 |
|------|------|
| **所有 mini-batch** | $e^{\mu}_{\text{old},t} \neq e^{\pi}_t$，**target policy 始终被改变**（始终有 bias） |

R3 **同时解决**两个问题：
- **Training-inference discrepancy**：训练侧使用的路由 = 推理侧的路由
- **Policy staleness**：路由被固定，不随 $\theta$ 漂移

R3 将 MoE 的训推 KL 散度降低到**接近 Dense 模型水平**。在数学推理 RL 任务中，无 R3 的训练在 60-120 步后崩溃，而 R3 训练全程稳定。

## R2 vs R3: 选择建议

GSPO 论文通过大规模对照实验（Qwen3-30B-A3B，数百千 GPU 小时）给出了以下结论：

| 训练场景 | 推荐 | 原因 |
|---------|------|------|
| **On-policy**（gbs = mbs） | 都不需要 | $\theta = \theta_{\text{old}}$，policy staleness ≈ 0，IS correction 即可 |
| **Off-policy 小**（gbs = 2×mbs） | **R2** | 首个 mini-batch 无 bias，足以稳定训练 |
| **Off-policy 大**（gbs ≥ 4×mbs） | **R3** | staleness 太大，必须同时修正训推不一致 |

> **核心 trade-off**：R2 更保守（仅修正引擎内部不一致，bias 更小）；R3 更激进（从根源消除所有路由不一致，但始终改变 target policy 引入 bias）。当 off-policiness 较小时，R3 的 bias 代价大于收益；当 off-policiness 较大时，不修正训推不一致的代价远大于 bias。
