...
注意这种情况下，相同轨迹的数据会拆分为多条数据，奖励相关的处理中，需要对相同轨迹的数据分配同样的reward。

可以在kwargs中获取 trajectory_inputs 获取完整轨迹的数据，具体实现参考[TODOREWARD类](https://github.com/modelscope/ms-swift/blob/main/examples/train/grpo/plugin/plugin.py)
...
通过kwargs获取`global_inputs`得到完整的全局rollout结果，根据 request_id 可以判断是否属于同一条轨迹，注意，返回的奖励需对应当前进程的数据。
