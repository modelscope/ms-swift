...
- 🔥load_from_cache_file: 是否从缓存中加载数据集，默认为False。
...
- freeze_parameters_patterns: 需要被冻结参数的正则表达式，默认为`[]`。
- freeze_parameters_ratio: 从下往上冻结的参数比例，默认为0。可设置为1将所有参数冻结，结合`trainable_parameters`设置可训练参数。
- trainable_parameters: 额外可训练参数的前缀，默认为`[]`。
- trainable_parameters_patterns: 匹配额外可训练参数的正则表达式，默认为`[]`。
  - 备注：`trainable_parameters`、`trainable_parameters_patterns`的优先级高于`freeze_parameters`、`freeze_parameters_patterns`和`freeze_parameters_ratio`。当指定全参数训练时，会将所有模块设置为可训练的状态，随后根据`freeze_parameters`、`freeze_parameters_patterns`、`freeze_parameters_ratio`将部分参数冻结，最后根据`trainable_parameters`、`trainable_parameters_patterns`重新打开部分参数参与训练。
