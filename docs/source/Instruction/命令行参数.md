...
- 🔥target_modules: 指定lora模块, 默认为`all-linear`, 自动寻找除lm_head外的linear并附加tuner. 你也可以指定`--target_modules all-linear all-embedding`来选中linear和embedding层. 该参数不限于LoRA
...
- modules_to_save: 在已附加tuner后，原模型参与训练和存储的模块，默认为`[]`. 你可以指定`all-embedding`来选中embedding层, `all-norm`来选中layernorm和rmsnorm层. 该参数不限于LoRA
