...
- 🔥resume_from_checkpoint: 断点续训参数，传入checkpoint路径。默认为None。断点续训请保持其他所有参数不变，额外增加`--resume_from_checkpoint checkpoint_dir`。
...
- write_batch_size: 结果写入`result_path`的batch_size。默认为1000。
