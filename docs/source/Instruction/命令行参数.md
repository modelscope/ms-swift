...
- ref_adapters: 默认为`[]`。
  - 注意：在"ms-swift>=3.8"，可以在训练时设置`--adapters sft_ckpt --ref_adapters sft_ckpt`，在该LoRA后进行继续训练，方便在LoRA SFT之后衔接DPO/KTO/GRPO的场景。若是此场景的断点续训，则设置`--resume_from_checkpoint dpo_ckpt --ref_adapters sft_ckpt`。
