...
- use_flash_ckpt:是否启用[DLRover Flash Checkpoint](https://github.com/intelligent-machine-learning/dlrover)的flash checkpoint默认为false，启用后，权重会先保存至共享内存，之后异步持久化；建议搭配PYTORCH_CUDA_ALLOC_CONF: "expandable_segments:True"一起使用，避免训练过程CUDA OOM。
