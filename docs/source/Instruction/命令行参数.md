...
>
> - **核心思想**：只更新模型的一小部分参数（如 LoRA、Prefix Tuning），而不是全部参数。
> - **优点**：
>   - 节省显存和计算资源
>   - 训练速度快
>   - 可以在多个任务上复用同一个基础模型
...
> 🔹 Unsloth
>
> - **核心思想**：在 PEFT 的基础上，进一步做极致优化，专注于**显存节省和训练加速**。
> - **优点**：
>   - 显存需求更低（相比普通 PEFT 再降 2-4 倍）
>   - 训练速度更快（使用更高效的 CUDA kernel、量化技巧）
>   - 和 HuggingFace/PEFT API 接口兼容，迁移成本低
> - **缺点**：
>   - 依赖特定优化库，不如纯 HuggingFace PEFT 那么通用
>   - 社区生态相对较新
