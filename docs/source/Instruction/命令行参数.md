...
- `--🔥ref_model_type`: 选择参考模型, 同model_type参数, 默认为`None`, 与训练模型一致。其中`cpo`, `simpo`, `orpo`算法无需选择。
...
- `--rpo_alpha`: 控制DPO中加入sft_loss的权重, 默认为0. 最后的loss为`KL_loss + rpo_alpha * sft_loss`.
- `--cpo_alpha`: CPO/SimPO loss 中 nll loss的系数, 默认为`None`, 如果使用`--rlhf_type cpo`, 则该值默认为`1.`, 如果是simpo, 默认为`0.`.
