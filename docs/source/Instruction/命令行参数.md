...
- interleave_prob: 默认为None。在多数据集组合时，默认使用`concatenate_datasets`函数，若设置该参数，则使用`interleave_datasets`函数。该参数通常在流式数据集组合时用到，传入interleave_datasets函数中
...
- sequence_parallel_size: 序列并行数量，默认为1。该参数只支持`swift pt/sft`。参考[example](https://github.com/modelscope/ms-swift/tree/main/examples/train/sequence_parallel/train.sh)
...
- train_dataloader_shuffle: 训练的dataloader是否随机，默认为True。该参数对IterableDataset无效。IterableDataset采用顺序的方式读取
