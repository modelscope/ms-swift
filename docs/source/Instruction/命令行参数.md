...
- data_flatten: 将一个batch中的数据进行展平而避免数据padding，从而降低显存占用并加快训练。默认为False。
  - 注意：使用data_flatten请结合`--attn_impl flash_attn`使用且"transformers>=4.44"，具体查看[该PR](https://github.com/huggingface/transformers/pull/31629)。（同packing）
  - 支持的多模态模型与多模态packing支持情况相同。
  - Megatron-SWIFT默认使用data_flatten，即`qkv_format='thd'`。
  - 比序列packing支持更多的训练方式：除了CPT/SFT，还支持DPO。
