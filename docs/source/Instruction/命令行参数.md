...
- model:SWIFT各功能需要的原始模型id或模型本地路径，如果是本地路径请配合`model_type`和`template`使用
- model_type:SWIFT定义的模型组，同样的模型架构、template、模型加载过程可以定义为一个组
- model_revision:模型版本
- torch_dtype:模型数据类型，支持`float16`,`bfloat16`,`float32`，默认从config文件中读取
...
### 数据参数
- dataset:数据集id或路径。支持如下方式:
  - 数据集id:子数据集#取样数量 空格分割传递多个
  - /本地路径文件.jsonl 空格分割传递多个
  - /本地包含训练文件的路径
- val_dataset:验证集id或路径，同dataset使用方式
- split_dataset_ratio:不指定val_dataset时如何拆分训练集和验证集
- data_seed:数据集随机种子
- dataset_num_proc:数据集预处理的进程数，默认值1
...
### 模板参数
- template:模板类型，参考[支持的模型和数据集](./支持的模型和数据集.md)，默认使用model对应的template类型。如果model为本地路径，请手动传入这个字段
- system:自定义system字段，默认使用template定义的system
- max_length:单行token最大长度
- truncation_strategy:如果超长如何处理，支持`delete`和`left`，代表删除和左侧裁剪，默认为左侧裁剪
- max_pixels:多模态模型图片前处理的最大像素数，默认不缩放
- tools_prompt:智能体训练时的工具列表转为system的格式，请参考[智能体训练](./智能体.md)
- loss_scale:如何针对训练添加token的loss权重。默认为`default`，代表所有response以1进行CE。具体可以查看[插件化](../Customization/插件.md)和[智能体训练](./智能体.md)
...
- template_backend:使用swift或jinja进行推理。如果使用jinja，则使用transformers的`apply_chat_template`
...
- quant_method:量化方法，支持`bnb`, `hqq`, `eetq`
- quant_bits:量化bit数，支持1,2,3,4,8
- hqq_axis:hqq量化axis
- bnb_4bit_compute_dtype:bnb量化计算类型，支持`float16`,`bfloat16`,`float32`
- bnb_4bit_quant_type: bnb量化类型，支持`fp4`和`nf4`，默认为`nf4`
- bnb_4bit_use_double_quant:是否使用双重量化，默认为`True`
- bnb_4bit_quant_storage:bnb量化存储类型，支持`float16`,`bfloat16`,`float32`，默认为None
...
以下参数来自transformers，SWIFT对其默认值进行了覆盖。SWIFT支持transformers trainer的所有参数，未列出的请参考[hf官方参数](https://github.com/huggingface/transformers/blob/main/src/transformers/training_args_seq2seq.py)
...
- disable_custom_all_reduce (bool): 是否禁用自定义的all-reduce kernel, 而回退到NCCL. 默认为`True`, 这与vLLM的默认值不同.
- enforce_eager (bool): vllm使用pytorch eager模式还是建立cuda graph. 默认为`False`. 设置为True可以节约显存, 但会影响效率.
...
- verbose:是否打印访问信息，默认`False`
- max_batch_size:最大batch_size，文本评测默认256，多模态默认16
- eval_url:评测一个url，需要配合model（访问的模型名称）/api_key（访问密码）两个参数使用
...
- quant_n_samples gptq/awq的校验集抽样数
- quant_seqlen:校验集sequence_length, 默认值2048
- quant_batch_size:量化batch_size，默认值1
- group_size:量化group大小，默认值128
