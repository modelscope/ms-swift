...
- rpo_alpha: 来自[RPO 论文](https://huggingface.co/papers/2404.19733)中的参数，用于控制损失函数中NLL项的权重（即SFT损失）。`loss = dpo_loss + rpo_alpha * sft_loss`。默认为`None`，即默认不引入sft_loss。
  - 注意：在"ms-swift<3.7.1"，其默认值为`1.`。在"ms-swift>=3.7.1"该默认值修改为`None`。
