...
其中的InferRequest来自于`swift.llm`，返回的`List[ChatCompletionResponse]`可以视作OpenAI的标准格式。开发者可以在infer_requests中拿到queries和responses，并按照自己的方式进行切分，例如：
```text
...
So, the answer is ...
```
开发者可以在这里对过程进行切分，并按batch传入PRM中进行推理并返回score。更通用来说，开发者可以在这里调用一个远端URL，例如一个闭源PRM大模型并返回score。
