...
agent模板的mapping文件可以参考[这里](https://github.com/modelscope/ms-swift/blob/main/swift/agent_template/mapping.py)。agent template设计目标是，使用一套数据集格式，可以实现不同模型训练的切换。训练时使用`--agent_template`指定对应的agent模板。

所有的AgentTemplate需要继承自BaseAgentTemplate，并实现其中的几个方法: `_format_tools`, `_format_tool_calls`, `_format_tool_responses`, `get_toolcall`。
...
- get_toolcall: 在部署的时候使用到，用于解析模型输出内容中的工具名和参数，返回`List[Function]`。
...
所有的回调类需继承自base.py中的TrainerCallback，并覆盖其方法。接口与transformers的TrainerCallback一致，请参考transformers的[callback文档](https://huggingface.co/docs/transformers/main_classes/callback)。


### Loss

Loss的mapping文件可以参考[这里](https://github.com/modelscope/ms-swift/blob/main/swift/loss/mapping.py)。
swift支持自定义loss（当前只支持sft/pretrain/reranker/embedding任务），注册后在训练时设置`--loss_type <loss-name>`使用你定制的loss方法。你可以参考`CustomCrossEntropyLoss`进行定制。

自定义Loss需要继承自BaseLoss，并实现`__call__`方法，返回标量。例如：
...
loss scale的mapping文件可以参考[这里](https://github.com/modelscope/ms-swift/blob/main/swift/loss_scale/mapping.py)。在pretrain和sft任务中，可训练token的loss是均匀的，即每个token平等地关注。但在某些情况下，某些token需要被额外关注，并设置更高的权重或者对某些部分不进行关注。loss_scale可以让开发者自由地定义自己的token权重。

你可以通过继承LossScale基类，并实现`get_loss_scale`方法来自定义loss scale。（预训练和SFT支持使用loss_scale控制token是否参与训练以及和其权重大小，RLHF中只支持控制token是否参与训练）
```python
class CustomLossScale(LossScale):
    ...
    def get_loss_scale(self, context: str, **kwargs) -> Tuple[List[str], List[float]]:
        ...
```
`get_loss_scale`函数需要返回了一个Tuple，第一个返回是拆解后的字符串的列表，第二个参数是字符串对应的loss_scale，float值代表了权重。例如下面的权重设置：
```text
["学习", "好", "数学", "是", "重要", "的"]
...
当然我们也需要关注`__call__`方法的核心逻辑，即loss_scale基础策略（base_strategy）: all/default/last_round对loss_scale的影响，具体参考[命令行参数文档](../Instruction/Command-line-parameters.md)的介绍。以及数据集中包含'loss'字段对loss_scale的影响，参考[自定义数据集文档](../Customization/Custom-dataset.md)。
```python
