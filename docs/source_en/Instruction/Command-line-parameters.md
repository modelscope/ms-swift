...
- streaming_shard: Splits the streaming training set across data-parallel ranks, so that each rank preprocesses only its own share instead of rank 0 preprocessing everything and scattering the batches. Default is `False`. Only effective when `--streaming true`.
  ...
  - Every rank now really uses its `--dataset_num_proc` encoder processes, instead of only rank 0 using them and the rest sitting idle. CPU and memory usage per node therefore scale with (ranks per node) x `dataset_num_proc`; lower `dataset_num_proc` if the node becomes CPU or memory bound.
  - The split is by blocks of consecutive samples, sized to the amount of raw data the loader consumes per item it emits (the per-device batch size, or `packing_interval` when packing). Rank `r` keeps every `world_size`-th block. Sizing it this way means each rank receives the samples rank 0 would have scattered to it: **without packing, each rank trains on exactly the same samples in the same order as with the default path**. With packing, each rank rebuilds its own packs, so the sample-to-pack grouping differs while the set of samples does not.
  - Cost: every rank reads the raw stream in full; only the expensive encoding is split. When the raw data is "jsonl with image paths + external images", the duplicated work is just cheap text parsing. When the raw data embeds large binaries (e.g. parquet with inlined image bytes), evaluate the duplicated read first, especially on multiple nodes.
  - The split is independent of the number and size of the underlying files, so per-rank sample counts differ by at most one block. All ranks stop as soon as one shard is exhausted, so each rank drops at most one step worth of trailing data (the default path drops a tail as well).
