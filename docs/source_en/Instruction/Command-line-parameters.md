...
- group_by_length_shuffle_batches: With `group_by_length=True`, shuffle complete DP micro-batch blocks after length grouping. Defaults to False. This reduces the repeated long-to-short ordering while preserving each local batch and cross-rank load balance. A block contains `per_device_train_batch_size * DP size` samples (`micro_batch_size * DP size` in Megatron), before gradient accumulation. The first block and incomplete tail stay in place. This does not mix samples within a block or guarantee better convergence. Keep this setting unchanged when resuming training. It applies to the map-style length-grouped samplers; streaming and the separate sequence-parallel sampler are unaffected.
