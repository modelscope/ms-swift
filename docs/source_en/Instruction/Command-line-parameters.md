...
- enable_thinking: Whether to enable thinking mode. Default is None (ms-swift>=3.12). During inference, the default value of this parameter is determined by the enable_thinking setting when registering the template: If enable_thinking is False, a non-thinking prefix is added, for example, the Qwen3-8B hybrid thinking model adds the prefix `<think>\n\n</think>\n\n`, while Qwen3-8B-Thinking does not add a prefix. If enable_thinking is True, a thinking prefix is added, for example `<think>\n`. During training, the default value of this parameter is determined based on the model type (True for thinking models, False for non-thinking models): If enable_thinking is True, a non-thinking prefix is added to data samples whose last round does not start with the thinking marker `<think>`. For example, the Qwen3-8B hybrid thinking model additionally adds `<think>\n\n</think>\n\n`, while Qwen3-8B-Thinking does not add a prefix. If enable_thinking is False, no such modification is made to the data samples.
