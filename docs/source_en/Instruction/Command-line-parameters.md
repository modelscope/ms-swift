...
- 🔥streaming: Stream reading and processing of the dataset, default is False. It is typically set to True when handling large datasets.
  - Note: You need to set `--max_steps` explicitly, as the stream dataset does not have a defined length. You can achieve training equivalent to `--num_train_epochs` by setting `--save_strategy true` and specifying a large enough `max_steps`. Alternatively, you can set `max_epochs` to ensure training stops after the specified number of epochs, at which point the model weights will be evaluated and saved.
