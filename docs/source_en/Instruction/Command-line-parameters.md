...
- 🔥result_path: Path to store inference results (jsonl). Defaults to None. When performing inference/deployment on a dataset, results are saved by default in the checkpoint directory (containing args.json file) or in the './result' directory. The final storage path will be printed in the command line.
