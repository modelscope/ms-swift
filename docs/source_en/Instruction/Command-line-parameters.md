...
- metric_for_best_model: Default is None, which means that when `predict_with_generate` is set to False, it is set to 'loss'; otherwise, it is set to 'rouge-l' (There is no default value setting during PPO and GRPO training).
