...
- 🔥cached_dataset: Use cached dataset (generated using `swift export --to_cached_dataset true ...` command) to avoid GPU time consumed by the tokenization process during large dataset training/inference. Default is `[]`. For examples, refer to [here](https://github.com/modelscope/ms-swift/tree/main/examples/export/cached_dataset).
  - Note: In "ms-swift>=3.11", the cached_dataset for multimodal models will only store an additional length field in the dataset (to avoid storage pressure, plain text models will store fields such as input_ids, length, etc.). Data samples that would cause errors will be filtered out during the export process. During training/inference, the `--max_length` parameter is supported for filtering/truncating oversized data and the `--packing` parameter is available. For multimodal models, the actual data preprocessing will be performed synchronously during training, and this process overlaps with training without affecting training speed.
