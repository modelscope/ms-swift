...
- 🔥streaming: Stream reading and processing of the dataset, default is False. It is typically set to True when handling large datasets.
  - Note: You need to set `--max_steps` additionally, because the length of a streaming dataset cannot be obtained. You can achieve training equivalent to `--num_train_epochs` by setting `--save_strategy true` and specifying a large value for max_steps.
