...
- kl_in_reward : determines whether the KL divergence is applied as a separate loss regularization (false) or incorporated directly into the reward by subtracting it from the reward signal (true), with its default value depending on the advantage estimator: false for grpo and true for rloo and reinforce_plus_plus.
- scale_rewards: Specifies the reward scaling strategy, computing standard deviation within groups for scaling. batch scales rewards across the entire batch, none applies no scaling. In ms-swift < 3.10, this was a boolean parameter where true corresponds to group and false to none. The default value is tied to the advantage_estimator: group for grpo, none for rloo, and batch for reinforce_plus_plus.
