...
- 🔥cached_dataset: Use cached dataset (generated using `swift export --to_cached_dataset true ...` command) to avoid GPU time consumed by the tokenization process during large dataset training/inference. Default is `[]`. For examples, refer to [here](https://github.com/modelscope/ms-swift/tree/main/examples/export/cached_dataset).
  - Note: In "ms-swift>=3.11", the cached_dataset for plain text models will store fields such as input_ids, labels, length, etc., while multimodal models will only store an additional length field (to avoid storage pressure). For multimodal models, the actual data preprocessing will be performed synchronously during training, and this process overlaps with training without affecting training speed. We recommend setting the same `--max_length` and `--truncation_strategy` parameters during both export and training (if set differently, we suggest reading the source code to understand the relevant details of plain text/multimodal data processing). During training/inference, we will remove data samples that exceed `--max_length`, and the `--packing` parameter is supported.
...
- truncation_strategy: How to handle samples whose tokens exceed `max_length`. Supports 'delete', 'left', 'right', and 'split', which represent deleting, left truncation, right truncation, and splitting into multiple data samples, respectively. The default is 'delete'. Among these, `--truncation_strategy split` is only supported during pretraining, i.e., in `swift/megatron pt` scenarios, and requires "ms-swift>=3.11". This strategy will split oversized fields into multiple data samples to avoid token waste.
