...
- streaming: Whether to stream and process the dataset on-the-fly. Default is `False`. (The shuffling of streaming datasets is not thorough, which may lead to severe loss fluctuations.)
  ...
  - Note: Streaming avoids waiting for preprocessing by overlapping it with training. However, preprocessing is by default only performed on rank 0 and then distributed to other processes. **This is typically less efficient than non-streaming data sharding**. When the training `world_size` is large, preprocessing and data distribution can become a bottleneck; set `--streaming_shard true` in that case.
- streaming_shard: Splits the streaming training set across data-parallel ranks, so that each rank preprocesses only its own share instead of rank 0 preprocessing everything and scattering the batches. Default is `False`. Only effective when `--streaming true`. All ranks must iterate the raw stream in the same deterministic order; the built-in dataset loader uses the same shuffle seed on every rank.
  - Preprocessing throughput then scales with the number of ranks. The gain is largest for multimodal data (image loading and encoding dominate) and for multi-node training, where the CPUs of every node other than rank 0's are otherwise idle. Within a single node the cores are shared either way, so there the gain comes from removing the rank 0 serialization rather than from extra cores — except without packing, where the default path encodes in a single process no matter how many cores the node has.
  - With packing, every rank now really uses its `--dataset_num_proc` encoder processes, instead of only rank 0 using them and the rest sitting idle. CPU and memory usage per node can therefore grow with (ranks per node) x `dataset_num_proc`; lower `dataset_num_proc` if the node becomes CPU or memory bound. Without packing, streaming encoding remains single-process per rank.
  - The split is by blocks of consecutive raw samples, sized to the amount of raw data the loader consumes per item it emits (the per-device batch size, or `packing_interval` when packing). Rank `r` keeps every `world_size`-th block. Without packing, this reproduces the default path's per-rank sample order **when preprocessing is deterministic and maps every raw sample to exactly one output**. Samples that are dropped (e.g. `truncation_strategy=delete` or bad media), expanded (`truncation_strategy=split`), or randomly augmented can change the per-rank encoded sequence.
  - With packing, each rank rebuilds its own packs and carries the leftover bin within that rank. Pack grouping, pack counts, and finite-stream tail coverage can therefore differ from the dispatch path.
  - Cost: every rank reads the raw stream in full; the template encoding and streaming packing stages are split. When the raw data is "jsonl with image paths + external images", the duplicated work is usually cheap text parsing. Expensive custom dataset preprocessors, or raw data that embeds large binaries (e.g. parquet with inlined image bytes), should be evaluated first, especially on multiple nodes.
  - The raw blocks are independent of the number and size of the underlying files, so per-rank raw sample counts differ by at most one block. The stopping policy is **first exhausted**: as soon as one rank runs out of encoded batches, all ranks stop together. This avoids a distributed hang but may leave an unbounded number of samples unconsumed on other ranks when post-filtering or packing produces imbalanced batch counts. Shuffling normally reduces systematic imbalance, and step-based training that stops at `max_steps` before exhaustion is unaffected.
  - Applies to the training set only; the validation set keeps using the default path.
  - GRPO and GKD are not supported because their encoding is deferred past the dataset post-processing stage.
  - Not supported together with `--deepspeed_autotp_size`: streaming does not replicate a batch across tensor-parallel ranks (this predates this parameter and also applies to the default path).
