...
- SWIFT_URL_ALLOWED_HOSTS: A comma-separated allowlist of hosts, e.g. `bucket.oss-cn-hangzhou.aliyuncs.com,cdn.example.com`. When set, media URLs are fetched only if their host is in this list, and those hosts are trusted regardless of the addresses they resolve to. Recommended when a deployment serves media from known bucket/CDN domains.
