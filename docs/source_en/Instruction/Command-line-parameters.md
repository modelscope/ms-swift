...
- reward_funcs: Reward functions in the GRPO algorithm; options include `accuracy`,`format`,`cosine`,`repetition` and `soft_overlong`, as seen in `swift/plugins/orm.py`. You can also customize your own reward functions in the plugin. Default is `[]`.
