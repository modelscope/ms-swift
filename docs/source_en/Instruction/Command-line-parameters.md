...
- early_stop_interval: The interval for early stopping. Training will be terminated when the best_metric shows no improvement within the number of steps specified by this parameter. The specific implementation can be found in the callback plugin. Additionally, if you have more complex early stopping requirements, you can directly override the existing implementation in [callback.py](https://github.com/modelscope/ms-swift/blob/main/swift/plugin/callback.py).
