...
- 🔥vllm_enable_prefix_caching: Enables vLLM's automatic prefix caching to save processing time for repeated prompt prefixes, improving inference efficiency. Defaults to `True` in colocate (GRPO training) mode, and `None` in other scenarios (deploy, inference, etc.), following vLLM's default behavior.
