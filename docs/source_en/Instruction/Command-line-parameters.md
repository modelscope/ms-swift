...
- agent_template: The Agent template determines how to convert the list of tools into the system, how to extract tool calls from model responses, and specifies the template format for `{"role": "tool", "content": "xxx"}`. It can be optional values such as "react_en", "reach_zh", "qwen_en", "qwen_zh", "hermes", "glm4", "toolbench", etc., with a default value of None, chosen based on the model type.
