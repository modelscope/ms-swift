...
- enable_thinking: Whether to enable thinking mode. Default is None (ms-swift>=3.12), the default value of this parameter is determined based on the model type (True for thinking/hybrid thinking models, False for non-thinking models). During inference, if enable_thinking is False, a non-thinking prefix is added, for example, the Qwen3-8B hybrid thinking model adds the prefix `<think>\n\n</think>\n\n`, while Qwen3-8B-Thinking does not add a prefix. If enable_thinking is True, a thinking prefix is added, for example `<think>\n`. During training, if enable_thinking is True, a non-thinking prefix is added to data samples whose assistant part does not start with the thinking marker `<think>`. For example, the Qwen3-8B hybrid thinking model additionally adds `<think>\n\n</think>\n\n`, while Qwen3-8B-Thinking does not add a prefix. If enable_thinking is False, no such modification is made to the data samples.
  - Note: During training, if loss_scale starts with last_round, this modification is only applied to the last round; if it does not start with last_round, for example if it is 'default', this modification is applied to every round of data.
  - Note: For thinking models (thinking/hybrid thinking), we will remove historical thinking content during both inference and training. If the loss_scale during training does not start with last_round, for example if it is 'default', then historical thinking content will not be removed.
...
- 🔥loss_scale: Loss weight configuration for training tokens. Default is `'default'`. loss_scale includes 3 basic strategies: 'default', 'last_round', 'all', and other strategies: 'ignore_empty_think' and agent-specific ones: 'react', 'hermes', 'qwen', 'agentflan', 'alpha_umi', etc. For available options, refer to [loss_scale.py](https://github.com/modelscope/ms-swift/blob/main/swift/plugin/loss_scale/loss_scale.py). ms-swift>=3.12 supports mixing basic strategies with other strategies, for example: `'default+ignore_empty_think'`, `'last_round+ignore_empty_think'`. If no basic strategy is specified, it defaults to 'default', for example: 'hermes' is equivalent to 'default+hermes'.
  ...
  - 'last_round': Only calculate loss for the last round response. In "ms-swift>=3.12", the last round means all content after the last "user", previously it only included the last "assistant". (**Default value for RLHF**)
