...
- SWIFT_ALLOW_INTERNAL_URL: Defaults to `'0'`. When fetching a media URL (image/audio/video), swift refuses URLs that resolve to non-public addresses (loopback, private ranges, etc.) to prevent SSRF, which is especially important for a `swift deploy` service reachable by untrusted callers. Set to `'1'` to allow such addresses when the media URLs are trusted (e.g. an internal image server used during offline training). Cloud metadata endpoints (`169.254.0.0/16`, Alibaba Cloud `100.100.100.200`, etc.) are always blocked regardless of this setting.
- SWIFT_URL_ALLOWED_HOSTS: A comma-separated allowlist of hosts, e.g. `bucket.oss-cn-hangzhou.aliyuncs.com,cdn.example.com`. When set, media URLs are fetched only if their host is in this list. An allowlisted host may resolve to an ordinary private address, but cloud metadata addresses remain blocked. Recommended when a deployment serves media from known bucket/CDN domains.
- SWIFT_MAX_DOWNLOAD_SIZE_MB: Maximum size of a single response body when fetching a media URL, defaulting to `1024` (i.e. 1GB). This prevents an untrusted caller from exhausting memory by pointing the server at a huge or endless response (the video path also writes a temporary file to disk). Set to `0` to disable the cap.
- SWIFT_MEDIA_ALLOWED_DIRS: A comma-separated allowlist of **absolute** directories, e.g. `/data/images,/data/videos` (a relative entry is resolved against the current working directory, which is rarely what you want). Unset by default, which keeps the current behavior (media files may be read from anywhere on disk, as training and local inference rely on). When set, a local file path resolved from a media string must live inside one of these directories, otherwise it is refused; paths are resolved (following symlinks and `..`) before comparison, so a symlink pointing outside the allowlist is refused as well. Recommended when `swift deploy` is exposed to untrusted callers: such a caller should only ever pass a URL or base64, and a local path would otherwise be read by the server and handed to the model, which describes its contents back -- effectively reading any file on the server. To refuse local paths outright, point this at an empty directory that holds nothing else. Note: training and local inference do not need this variable by default; if you set it in a training environment as well (e.g. training and deployment share the same environment), add the directories holding your training data to the allowlist too, otherwise loading local media outside the allowlist during training is refused.
