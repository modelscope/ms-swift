...
#### GKD Arguments
- lmbda: Default is 0.5. This parameter is used in GKD. It controls the probability of on-policy student generation. With probability `lmbda`, the student model generates responses; otherwise, dataset responses are used. Set `lmbda=0` for pure off-policy (dataset only), `lmbda=1` for pure on-policy.
