...
- Models trained with QLoRA will have significant errors when merging LoRA (BNB/HQQ supports merge-lora, while GPTQ/AWQ does not). It is not recommended to use QLoRA for fine-tuning. Instead, it is recommended to fine-tune using LoRA or full parameters, and then merge into complete weights before using GPTQ/AWQ/BNB for [quantization](https://github.com/modelscope/ms-swift/tree/main/examples/export/quantize).
