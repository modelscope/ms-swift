...
- By default, `GatedDeltaNet` uses the transformers implementation (to ensure stability, the default behavior remains unchanged for now). To use mcore's GDN for lower memory usage and faster speed, please upgrade to `megatron-core>=0.16` and set the environment variable `SWIFT_USE_MCORE_GDN=1`.
