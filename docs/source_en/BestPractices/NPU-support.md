...
1. The MindSpeed training combination currently pinned by the NPU documentation is `Megatron-LM v0.16.0 + MindSpeed core_r0.16.0`. `mcore-bridge` defaults to the Megatron-Core/MindSpeed GDN path with `USE_MCORE_GDN=1`. With `USE_MCORE_GDN=0`, it falls back to the transformers GDN. That path prefers the FLA Triton-Ascend backend described above; when FLA is unavailable, it logs a warning and falls back to the native Torch GDN, so FLA is not a hard dependency for ordinary Megatron training.
