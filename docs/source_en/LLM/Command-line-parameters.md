...
- `loss_scale_config_path`: Effective when use_loss_scale is set to True, this option allows setting a custom loss_scale configuration, often used in conjunction with Agent training to amplify the loss of Action and other ReAct fields.
  - You can configure loss_scale in the form of a dictionary where keys correspond to special fields, and values indicate the loss_scale for the special field and the fields that follow. For instance, with `"Observation:": [2, 0]`, if the response is `xxxx Observation:error`, the loss for the `Observation:` part will be doubled in the loss calculation, and the `error` part will not be considered. Patterns can also be matched using regular expressions, with the key being the matching pattern and the value representing the loss_scale for the matched field, e.g., `"r'<.*?>'":[1.0]`.
  - It also supports matching queries to set the loss_scale for an entire response. For example, for fixed multi-turn dialogue queries involved in the [Agent-Flan paper](https://arxiv.org/abs/2403.12881), we can match fixed queries to set it. You can refer to the example in `swift/llm/agent/agentflan.json`.
  - The default configuration uses `swift/llm/agent/default_loss_scale_config.json`, which applies loss scaling to the `Action:`, `Action Input:`, `Thought:`, `Final Answer:`, and `Observation:` fields.
