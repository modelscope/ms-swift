...
- optimizer: Optimizer type. Options include 'adam', 'sgd', 'muon', and 'dist_muon'. Default is 'adam'.
  ...
  - Note: 'muon' and 'dist_muon' are incompatible with `vit_lr`/`aligner_lr`, because those parameters take over the parameter grouping, which prevents Muon from routing the non-matrix parameters to `muon_scalar_optimizer`.
