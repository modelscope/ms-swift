...
if TYPE_CHECKING:
    from swift.llm.infer.protocol import ChatCompletionResponse, RolloutResponseChoice, RequestConfig, GymRolloutResponseChoice
...
class RolloutOutput:
    ...
    # multi turn rollout
    messages: Optional['Messages']  # history of messages for the rollout
    response_token_ids: Optional[List[List[int]]]  # response token ids for each rollout turn
    response_loss_scale: Optional[List[List[int]]]  # response loss scale for each rollout turn
    extra_info: Optional[Dict[str, Any]]  # make sure serializable
...
class RolloutScheduler(ABC):
    ...
    async def async_infer(self,
        ...
        async def _infer_async_single(infer_request: Union['RolloutInferRequest', Dict[str, Any]],
            ...
            from swift.llm.infer.protocol import RolloutInferRequest
    ...
    async def run(self, infer_request: 'RolloutInferRequest', request_config: 'RequestConfig',
        ...
        response_loss_scale = [1] * len(response_token_ids)
        return RolloutOutput(
            results=result,
            messages=infer_request.messages,
            response_token_ids=[response_token_ids],
            response_loss_scale=[response_loss_scale],
...
class MultiTurnScheduler(RolloutScheduler, ABC):
    ...
    Options:
        - If each round's response token ids are included in the RolloutOutput,
        ...
        - If both response token ids and response loss_scale are returned in the RolloutOutput,
          you can manually control the loss mask by specifying a loss_scale value for each token.
          The Trainer will use the provided loss_scale values directly when computing the loss.
          Note: Returning loss_scale requires that response token ids are also returned,
    ...
    Loss mask configuration:
        ...
        2. Return response token ids along with a corresponding response loss scale tensor (of equal length) to indicate the loss mask for each token.
    """
    ...
    def step(self, infer_request: 'RolloutInferRequest', result: 'RolloutResponseChoice',
        ...
        Returns:
            Either:
            - The next inference request, OR
            - A tuple of (next_request, info_dict) where info_dict contains
              additional metadata to be stored in the final result
        """
