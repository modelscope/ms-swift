...
def convert_hf2mcore(args: ExportArguments) -> None:
    hf_model, template = prepare_model_template(args, patch_offload=True)
...
def convert_mcore2hf(args: ExportArguments) -> None:
    ...
    hf_model, template = prepare_model_template(args, load_model=args.to_hf, patch_offload=True)
    processor = template.processor
