...
class MegatronExport(SwiftPipeline):
    ...
    def convert_mcore2hf(self) -> None:
        ...
        if args.test_convert_precision:
            with disable_safe_ddp_context_use_barrier():
                ...
                hf_model, template = prepare_model_template(args, device_map='cpu', **kwargs) if is_last_rank() else (None, template)
    ...
    def convert_hf2mcore(self) -> None:
        ...
        if args.test_convert_precision:
            with disable_safe_ddp_context_use_barrier():
                hf_model, template = prepare_model_template(args, device_map='cpu') if is_last_rank() else (None, template)
            test_convert_precision(hf_model, mg_model, template, args.test_convert_dtype)
            dist.barrier()
