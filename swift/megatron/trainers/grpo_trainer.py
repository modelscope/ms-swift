...
from swift.megatron.utils import (RouterReplay, RouterReplayAction, RouterReplayHelper, forward_step_helper,
                                  get_local_topk_idx_for_current_rank, get_padding_to, get_router_replay_data,
                                  set_random_seed, set_router_replay_data)
...
class MegatronGRPOTrainer(MegatronRolloutMixin, MegatronRLHFTrainer):
    ...
    def _maybe_compute_logps(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output = self.model_forward(self.unwrapped_models[0], iter([deepcopy(inputs)]), no_grad=True, per_token=True)
    ...
    def model_forward(self, model, data_iterator, no_grad=True, per_token=False):
        ...
        if RouterReplayHelper.is_replay_forward_action(model.config):
            ...
            layers_topk_idx = get_local_topk_idx_for_current_rank(global_topk_idx, model.config,
