...
class MegatronGDPOTrainer(MegatronRolloutMixin, MegatronRLHFTrainer):
    ...
    def _prepare_rewards(self):
        ...
        if self._async_reward_func_indices:
            self.async_reward_loop_thread, self.async_reward_loop, self.async_reward_loop_ready_event = (
                start_event_loop_in_daemon(name='MegatronGDPOTrainer-AsyncRewardLoop'))
    ...
    def _compute_advantages(self,
        ...
        # GDPO: normalize each reward separately, then sum
        # Step 1: Group-level normalization for each reward function
        normalized_advantages_list = []
        for i in range(num_reward_funcs):
            reward_i = total_rewards_per_func[:, i]  # [total_samples]
            grouped_reward_i = reward_i.view(-1, K)  # [num_prompts, K]
            ...
            normalized_i = (grouped_reward_i - group_mean) / group_std  # [num_prompts, K]
            normalized_i = normalized_i.view(-1)  # [total_samples]
            
            # Apply reward weight
            normalized_advantages_list.append(self.reward_weights[i] * normalized_i)
        
        # Step 2: Sum normalized advantages
        summed_advantages = sum(normalized_advantages_list)  # [total_samples]
        ...
        # Apply KL penalty if kl_in_reward is enabled
        if self.kl_in_reward and self.beta != 0.0 and kl_values is not None:
            self._metrics[mode]['kl'].append(kl_values.nanmean().item())
            advantages = advantages - self.beta * kl_values
        
        # Compute rewards for logging (sum of weighted rewards, not normalized)
