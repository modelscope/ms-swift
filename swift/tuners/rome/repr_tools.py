...
def get_words_idxs_in_templates(
    tokenizer: AutoTokenizer, context_templates: List[str], words: List[str], subtoken: str
) -> List:
    """
    ...
    for prefix, word, suffix in zip(prefixes, words, suffixes):
        ...
        if subtoken == "last" or subtoken == "first_after_last":
            lens.append([
                    ...
                    - (1 if subtoken == "last" or suffixes_len[i] == 0 else 0) - len(prefix_word_suffix_token)
                ])
        elif subtoken == "first":
...
def get_reprs_at_idxs(
    ...
    for batch_contexts, batch_idxs in _batch(n=512):
        contexts_tok = tokenizer(batch_contexts, padding=True, return_token_type_ids=False, return_tensors="pt").to(
