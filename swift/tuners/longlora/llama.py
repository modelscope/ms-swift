...
def _preprocess_qkv_fa2(attn_module, query_states, key_states, value_states, attention_mask):
    if attn_module.training:
        ...
        key_states = shift(key_states, bsz, q_len, group_size, attn_module.num_heads,
...
def _postprocess_qkv(attn_module, attn_output, q_len):
    if attn_module.training:
        ...
        attn_output[:, :, attn_module.num_heads
                    // 2:] = attn_output[:, :, attn_module.num_heads // 2:].roll(
                        group_size // 2, dims=1)
...
# code borrowed from https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L316
def eager_forward(
    self,
    ...
    **kwargs,
    ...
    query_states, key_states, value_states, attention_mask = _preprocess_qkv(self,
...
# code borrowed from https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L417
def fa2_forward(
    self,
    hidden_states: torch.Tensor,
    ...
    **kwargs,
    ...
    # patch position rolling
    query_states, key_states, value_states, attention_mask = _preprocess_qkv_fa2(self,
        query_states, key_states, value_states, attention_mask)
...
# code borrowed from https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L605
def sdpa_forward(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    ...
    # patch position rolling
    query_states, key_states, value_states, causal_mask = _preprocess_qkv(self,
