...
def dataset():
    ...
    with gr.Accordion(elem_id='quantization_tab', open=False):
        with gr.Row():
            quantization_bit = gr.Slider(elem_id='quantization_bit', minimum=0, maximum=8, step=4)
            bnb_4bit_comp_dtype = gr.Dropdown(elem_id='bnb_4bit_comp_dtype', choices=get_choices('bnb_4bit_comp_dtype'))
            bnb_4bit_quant_type = gr.Dropdown(elem_id='bnb_4bit_quant_type', choices=get_choices('bnb_4bit_quant_type'))
            bnb_4bit_use_double_quant = gr.Checkbox(elem_id='bnb_4bit_use_double_quant')
