...
def model():
    ...
    with gr.Row():
        model_type = gr.Dropdown(elem_id='model_type', choices=list(MODEL_MAPPING.keys()), scale=20)
        model_id_or_path = gr.Textbox(elem_id='model_id_or_path', lines=1, scale=20)
        template_type = gr.Dropdown(elem_id='template_type', choices=list(TEMPLATE_MAPPING.keys()) + ['AUTO'], scale=20)
    with gr.Row():
    ...
    model_type.change(update_input_model, inputs=[model_type], outputs=[model_id_or_path, system, template_type])
