...
def model():
    ...
    with gr.Row():
        model_type = gr.Dropdown(
            elem_id='model_type', choices=list(MODEL_MAPPING.keys()) + [locales['checkpoint']],
            value=locales['checkpoint'],
            scale=20)
    ...
    def update_input_model(choice):
        ...
        return model_id_or_path, default_system, template, gr.update(interactive=choice == locales['checkpoint'])

    def update_interactive(choice):
        return gr.update(interactive=choice == locales['checkpoint'])
