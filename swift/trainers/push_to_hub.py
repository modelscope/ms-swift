...
def create_repo(repo_id: str, *, token: Union[str, bool, None] = None, private: bool = False, **kwargs) -> RepoUrl:
    ...
    hub_model_id = PushToHubHelper._create_ms_repo(repo_id, token, private)
    PushToHubHelper.ms_token = token
    with tempfile.TemporaryDirectory() as temp_cache_dir:
        repo = Repository(temp_cache_dir, hub_model_id)
        PushToHubHelper._add_patterns_to_gitattributes(repo, ['*.safetensors', '*.bin', '*.pt'])
        # Add 'runs/' to .gitignore, ignore tensorboard files
        PushToHubHelper._add_patterns_to_gitignore(repo, ['runs/', 'images/'])
        PushToHubHelper._add_patterns_to_file(
            repo,
            'configuration.json', ['{"framework": "pytorch", "task": "text-generation", "allow_remote": true}'],
            ignore_push_error=True)
        # Add '*.sagemaker' to .gitignore if using SageMaker
        if os.environ.get('SM_TRAINING_ENV'):
            PushToHubHelper._add_patterns_to_gitignore(repo, ['*.sagemaker-uploading', '*.sagemaker-uploaded'],
...
def upload_folder(
    ...
    **kwargs,
    ...
    push_to_hub(
        repo_id,
        folder_path,
        token or PushToHubHelper.ms_token,
        commit_message=commit_message,
...
class PushToHubHelper:
    ms_token = None

    @staticmethod
    def patch():
        if not strtobool(os.environ.get('USE_HF', 'False')):
    ...
    def _add_patterns_to_gitignore(repo, patterns: List[str], commit_message: Optional[str] = None) -> None:
        PushToHubHelper._add_patterns_to_file(repo, '.gitignore', patterns, commit_message, ignore_push_error=True)
    ...
    def _add_patterns_to_gitattributes(repo, patterns: List[str], commit_message: Optional[str] = None) -> None:
        ...
        if commit_message is None:
            commit_message = f'Add `{patterns[0]}` patterns to {file_name}'
        PushToHubHelper._add_patterns_to_file(repo, file_name, new_patterns, commit_message, ignore_push_error=True)
