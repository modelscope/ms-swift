...
class GenerationArguments:
    ...
    Args:
        ...
        top_p (float): The cumulative probability for nucleus sampling. Filters the vocabulary to the smallest set of
            tokens whose cumulative probability exceeds `top_p`. Defaults to None (reads from
            'generation_config.json').
        repetition_penalty (float): The penalty applied to repeated tokens. A value of 1.0 means no penalty. Defaults
            to None (reads from 'generation_config.json').
        num_beams (int): The number of beams to use for beam search. Defaults to 1.
        ...
        top_logprobs (int): The number of top log probabilities to return for each token position. Requires `logprobs`
