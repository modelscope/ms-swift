...
class AttentionType:
    ...
    def __init__(self, use_flash_attn, attn_type):
        self.attn_type = attn_type
        self.convert_use_flash_attn(use_flash_attn)

    def convert_use_flash_attn(self, use_flash_attn):
        if use_flash_attn is True:
            self.attn_type = 'flash_attention'
        elif use_flash_attn is False:
            self.attn_type = 'eager'

    def to_bool(self):
        if self.attn_type == AttentionType.flash_attention:
            return True
        elif self.attn_type in (AttentionType.eager, AttentionType.sdpa):
            return False
        else:
            return False

    def to_impl(self, attn_version):
        if self.attn_type == AttentionType.flash_attention:
            return 'flash_attention_2' if attn_version == 2 else 'flash_attention_3'
        elif self.attn_type == AttentionType.eager:
            return AttentionType.eager
        else:
...
@register_model(
    ...
    template=TemplateType.wizardlm2_awq,
    requires=['transformers>=4.34'],
...
@register_model(
    ...
    template=TemplateType.numina_math,
    support_flash_attn=True,
    support_vllm=True,
    tags=['math'],
...
@register_model(
    ...
    template=TemplateType.openbuddy2,
    support_flash_attn=True,
    support_vllm=True,
    support_lmdeploy=True,
...
@register_model(
    ...
    template=TemplateType.reflection,
    support_flash_attn=True,
    support_vllm=True,
    requires=['transformers>=4.43'],
...
@register_model(
    ...
    template=TemplateType.longwriter_llama3,
    support_flash_attn=True,
    support_vllm=True,
    support_lmdeploy=True,
    requires=['transformers>=4.43'],
...
@register_model(
    ...
    'qwen/Qwen-Audio',
    template=TemplateType.qwen_audio_generation,
    support_flash_attn=True,
    function_kwargs={'get_qwen_function': get_model_tokenizer_qwen_base},
    tags=['multi-modal', 'audio'],
