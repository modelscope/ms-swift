...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def _stat_dataset(self, dataset: HfDataset):
        args = self.args
        dataset = GetLengthPreprocessor()(dataset, num_proc=args.dataset_num_proc, enable_cache=args.enable_cache)
    ...
    def _encode_dataset(self, train_dataset, val_dataset):
        ...
        if args.lazy_tokenize:
            ...
            train_dataset = preprocessor(
                train_dataset, num_proc=args.dataset_num_proc, strict=args.strict, enable_cache=args.enable_cache)
            if val_dataset is not None and not args.predict_with_generate:
                val_dataset = preprocessor(
                    val_dataset, num_proc=args.dataset_num_proc, strict=args.strict, enable_cache=args.enable_cache)
