...
class EvalModel(CustomModel):
    ...
    async def call_openai_batched(
        self, prompts: List[str], request_config: XRequestConfig
    ) -> List[str]:
    ...
    def predict(self, prompts: List[str], **kwargs) -> List[Dict[str, Any]]:
        ...
        if self.args.eval_url is not None:
            ...
            response_list = asyncio.run(
                self.call_openai_batched(prompts, request_config)
            )
            ...
            prog_bar = tqdm(
                total=len(prompts), dynamic_ncols=True, disable=not use_tqdm
            )
            for prompt in prompts:
    ...
    def __enter__(self):
        ...
        if os.path.exists(local_dir) and not os.path.islink(local_dir):
            raise AssertionError(
                "Please promise your pwd dir does not contain a `data` dir."
            )
...
def get_model_type(port, timeout):
    ...
    while True:
        ...
        try:
            ...
            if cnt > timeout:
                logger.error(
                    "Cannot get model_type from the deploy service, please check the error to continue eval"
                )
                raise e
            else:
                time.sleep(1)
...
def opencompass_runner(
    args: EvalArguments, dataset: List[str], model_type: str, is_chat: bool, url: str
):
...
def eval_opencompass(args: EvalArguments) -> List[Dict[str, Any]]:
    ...
    with deploy_context(args):
        if not args.eval_url:
            ...
            model_type = (
                "default-lora"
                if args.sft_type in ("lora", "longlora") and not args.merge_lora
        ...
        nlp_datasets = set(OpenCompassBackendManager.list_datasets()) & set(
            args.eval_dataset
        )
        mm_datasets = set(VLMEvalKitBackendManager.list_supported_datasets()) & set(
            args.eval_dataset
        )
    ...
    if not final_report:
        raise ValueError(
            f"Cannot load final report, please check your dataset: {args.eval_dataset} and the eval log"
        )
    return final_report
...
def eval_llmuses(args: EvalArguments) -> List[Dict[str, Any]]:
    ...
    if result_dir is None:
        result_dir = (
            eval_model.llm_engine.model_dir
            if args.infer_backend == "vllm"
            else eval_model.model.model_dir
        )
