...
def add_vllm_request(
    llm_engine: Union[LLMEngine, AsyncLLMEngine],
    inputs: Dict[str, Any],
    *,
    request_id: str,
        ...
        for key in ["images", "audios", "videos"]:
            ...
            if meida_data:
                if version.parse(vllm.__version__) < version.parse("0.6"):
                    ...
                    mm_data = {
                        key.rstrip("s"): (
                            meida_data[0] if len(meida_data) == 1 else meida_data
                        )
                    }
        if mm_data:
            llm_inputs["multi_modal_data"] = mm_data
        if llm_engine.__class__.__name__ == "LLMEngine":
            result_generator = llm_engine.add_request(
                request_id, llm_inputs, generation_config, **kwargs
            )
        else:
            result_generator = llm_engine.generate(
                llm_inputs, generation_config, request_id, **kwargs
            )
    else:
        if llm_engine.__class__.__name__ == "LLMEngine":
            result_generator = llm_engine.add_request(
                request_id, None, generation_config, input_ids, **kwargs
            )
        else:
            result_generator = llm_engine.generate(
                None, generation_config, request_id, input_ids, **kwargs
            )
...
def _prepare_vllm_request(
    ...
    *,
    ...
    use_tqdm: bool = False,
    **kwargs,
) -> Tuple[List[Optional[Dict[str, Any]]], List[Tuple[bool, int]]]:
    ...
    if (
        ...
        and tokenizer.eos_token not in generation_config.stop
    ):
        generation_config.stop.append(tokenizer.eos_token)
    if (
        isinstance(template.suffix[-1], str)
        and template.suffix[-1] not in generation_config.stop
    ):
    ...
    def _prepare_inputs(request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # agent support
        is_observation = (
            history[-1][-1].endswith("Observation:")
    ...
    with template.vllm_context(), concurrent.futures.ThreadPoolExecutor(
        max_workers=min(max_workers, len(request_list))
    ) as executor:
        futures = [
            executor.submit(_prepare_inputs, request) for request in request_list
        ]
...
def inference_stream_vllm(
    ...
    **kwargs,
    ...
    if generation_config is None:
        generation_config = (
            getattr(llm_engine, "generation_config", None) or VllmGenerationConfig()
        )
    ...
    prog_bar = tqdm(
        total=generation_info["num_samples"], dynamic_ncols=True, disable=not use_tqdm
    )
    while llm_engine.has_unfinished_requests():
        ...
        for output in step_outputs:
            ...
            generation_info["num_generated_tokens"] += (
                n_gen_tokens - num_generated_tokens[i]
            )
...
def inference_vllm(
    ...
    **kwargs,
    ...
    prog_bar = tqdm(
        total=generation_info["num_samples"] - old_num_samples,
        dynamic_ncols=True,
    ...
    for output in outputs:
        ...
        generation_info["num_generated_tokens"] += sum(
            len(_output.token_ids) for _output in output.outputs
        )
...
def prepare_vllm_engine_template(
    args: InferArguments, use_async: bool = False
) -> Tuple[LLMEngine, Template]:
    logger.info(f"device_count: {torch.cuda.device_count()}")

    assert not (
        args.sft_type == "lora" and not args.vllm_enable_lora
    ), "you need to merge lora"
