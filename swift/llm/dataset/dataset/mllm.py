...
from ..preprocess import ResponsePreprocessor, RowPreprocessor
from ..register import DatasetMeta, SubsetDataset, register_dataset
from ..constant import MLLMDatasetName
import json
from datasets import Dataset as HfDataset, IterableDataset, concatenate_datasets, interleave_datasets
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=GPT4vDataset(columns_mapping={'link': 'images'}),
        subsets=['default'],
        split=['train'],
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=ResponsePreprocessor(columns_mapping={
            'instruction': 'system', 'inputs': 'query', 'image_base64_str': 'images', 'outputs': 'response'}),
        split=['train'],
        huge_dataset=True,
        tags=['chat', 'multi-modal', 'vision']))
...
register_dataset(
    DatasetMeta(
        ...
        preprocess_func=RLAIFVPreprocessor(columns_mapping={'image': 'images'}),
