...
def parse_sample_configs(argv: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    classes = [ModelConfig, TemplateConfig, DatasetConfig, DistributedConfig, CheckpointConfig, TunerConfig,
               GenerationConfig, RolloutConfig, InferConfig, SamplingConfig, RLHFConfig, QuantizeConfig,
               RuntimeConfig]
    owners = {
    ...
    configs = parse_configs_strict(
        classes, effective_argv, command='swift sample', field_owners=owners, load_args_default=True)
...
def sample_main(argv: Optional[List[str]] = None) -> str:
    ...
    process_and_validate_configs(
        configs, add_version=False, resolve_model=sampling.sampler_engine not in {'client', 'no'})
