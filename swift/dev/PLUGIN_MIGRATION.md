...
> 一句话结论：**扩展点由 swift 自己声明、自己给基类、自己解析名字**（`swift/dev/plugin.py`），twinkle 只收到构造好的对象；规则奖励与 legacy `orms` 共享注册表，奖励模型通过兼容适配层消费 legacy `rm_plugins`，多轮状态机由 Twinkle 驱动，dev 仅适配 legacy scheduler/env 注册表。其余各类要么归到内核（loss），要么在 dev 里**确实没有消费方**——后者被 `swift/dev/tests/component/plugin/test_invariants.py` 逐条钉住，带原因，不允许悄悄忽略。
...
| `rm_plugins` | `swift/rewards/rm_plugin.py` | 2 | **已接入（兼容层）** | GRPO 为每个 `reward_model` 构建独立 `reward_template`，`swift/dev/reward.py::build_reward_model_plugins` 将 twinkle 模型适配到 legacy plugin 契约；规则奖励列在前、模型奖励列在后，共同应用 `reward_weights` |
...
| `multi_turns` | `swift/rollout/multi_turn.py` | 4 | **已接入（兼容层）** | `run_grpo` 将 `RLHFConfig.multi_turn_scheduler` 交给 `swift/dev/rollout/multi_turn.py::MultiTurnRollout`；Twinkle 统一驱动批量多轮循环、token budget、终止和结果聚合，dev 只转换 legacy scheduler 协议、展开每个 prompt 的独立 trajectory，并保留真实 token IDs、loss mask、rollout logprobs 与完整 messages |
| `envs` | `swift/rollout/gym_env.py` | 1 | **已接入（兼容层）** | `gym_env`/`use_gym_env` 选择 `GYMScheduler` 及 env；Twinkle 驱动循环，dev adapter 为每条 trajectory 分配独立 UUID 并转发 scheduler hook，reset/step/close 生命周期仍由 scheduler 管理，累计 `total_reward` 作为 GRPO 最后一列奖励并参与 `reward_weights` |
...
| 3 | callback / metric | dev 侧还没有可挂载的位置，登记在 `UNWIRED` 表 |
