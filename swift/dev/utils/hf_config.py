...
"""``HfConfigFactory``: read/write config attributes across nested HF configs.

Internalized from ``swift.utils.hf_config`` so the dev stack no longer couples to legacy. The class
is a self-contained bundle of ``@staticmethod`` config helpers whose only dependency is
``deep_getattr`` (also internalized in :mod:`swift.dev.utils.utils`).
"""
import torch
