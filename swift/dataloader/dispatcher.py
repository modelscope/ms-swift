...
class DataLoaderDispatcher:
    ...
    def _shard_block_size(self) -> int:
        ...
        each rank ends up with the blocks rank 0 would have scattered to it when preprocessing
        is one-to-one. With packing it only lines the packing windows up -- rank 0 scatters
        individual packs, not whole packing rounds -- so neither the pack grouping nor the
        amount of trailing data consumed is guaranteed to match the dispatch path.
        """
