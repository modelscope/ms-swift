...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _compute_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
        ...
        def log_rewards_metrics(rewards: torch.Tensor, rewards_per_func_for_metrics: torch.Tensor):
            ...
            if self.num_generations > 1:
                ...
                # Single generation mode (REINFORCE)
                rewards_mean = rewards.mean().item()
                rewards_std = rewards.std().item() if len(rewards) > 1 else 1.0
                is_std_zero = torch.tensor([False])
        ...
        if self.kl_in_reward and self.beta != 0.0:
            ...
            rewards = rewards 
        # Handle single generation case (REINFORCE)
        if self.num_generations == 1:
            # For REINFORCE, advantages are just the rewards themselves
            #
            advantages = rewards
            
            # Log metrics
            log_rewards_metrics(rewards=rewards, rewards_per_func_for_metrics=rewards_per_func)
    ...
    def _compute_loss_and_metrics(self, model, inputs):
        ...
        if self.template.padding_free:
            ...
            per_token_loss = -coef_1 * advantages.unsqueeze(0)
        else:
            per_token_loss = -coef_1 * advantages.unsqueeze(1)
        if entropy_mask is not None:
            per_token_loss = per_token_loss * entropy_mask
        # 移除KL loss，只保留GRPO核心loss
        # if per_token_kl is not None:
        #     per_token_loss = per_token_loss + self.beta * per_token_kl

        print(f"DEBUG: loss_type={self.loss_type}, padding_free={self.template.padding_free}")
        print(f"DEBUG: per_token_loss.shape={per_token_loss.shape}, completion_mask.shape={completion_mask.shape}")
        if self.template.padding_free:
            print(f"DEBUG: lengths.shape={lengths.shape}")
