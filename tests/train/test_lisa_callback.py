...
def test_lisa_can_be_created_before_trainer_initialization(tmp_path):
...
def test_lisa_does_not_freeze_layers_before_optimizer_creation(tmp_path):
    model = TinyLayerModel()
    args = make_args(tmp_path)
...
def test_newly_activated_lisa_layers_receive_optimizer_updates(tmp_path, monkeypatch):
    torch.manual_seed(42)
    model = TinyLayerModel()
    args = make_args(tmp_path)
    active = [0]
    monkeypatch.setattr('swift.callbacks.lisa.np.random.choice', lambda *args, **kwargs: [active[0]])
...
def test_lisa_rejects_adapter_training(tmp_path):
    args = make_args(tmp_path)
    args.tuner_type = 'lora'
    with pytest.raises(AssertionError, match='full parameter training'):
