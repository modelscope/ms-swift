...
def test_qwen_audio():
    pt_engine = TransformersEngine('Qwen/Qwen-Audio-Chat')
    _infer_model(pt_engine)


def test_qwen2_audio():
    # transformers==4.48.3
    pt_engine = TransformersEngine('Qwen/Qwen2-Audio-7B-Instruct')
...
def test_xcomposer2d5_ol():
    pt_engine = TransformersEngine('Shanghai_AI_Laboratory/internlm-xcomposer2d5-ol-7b:audio')
    _infer_model(pt_engine)
    pt_engine.default_template.template_backend = 'jinja'
    _infer_model(pt_engine)
...
def test_step_audio_chat():
    pt_engine = TransformersEngine('stepfun-ai/Step-Audio-Chat')
...
def test_gemma3n():
    pt_engine = TransformersEngine('google/gemma-3n-E4B-it')
...
def test_midashenglm():
    pt_engine = TransformersEngine('mispeech/midashenglm-7b')
...
def test_step_audio2_mini():
    pt_engine = TransformersEngine('stepfun-ai/Step-Audio-2-mini')
    messages = [{'role': 'user', 'content': '<audio>Caption the audio'}]
    response = _infer_model(pt_engine, messages=messages)
