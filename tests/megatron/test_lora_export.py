...
def report(stat: DiffStat, tag: str):
    ...
    print(f"[{tag}] max|Δ|={stat.max_abs:.3e}  mean|Δ|={stat.mean_abs:.3e}  cos={stat.cos_sim:.6f}  -> {'OK' if ok else 'MISMATCH'}")
    return ok
...
def load_models():
    ...
    merged = AutoModelForCausalLM.from_pretrained(
        MERGED_MODEL, torch_dtype=DTYPE, device_map={"": GPU_MERGED}
    ).eval()

    base = AutoModelForCausalLM.from_pretrained(
        BASE_MODEL, torch_dtype=DTYPE, device_map={"": GPU_PEFT}
    ).eval()
