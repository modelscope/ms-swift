from swift.llm import get_template, InferRequest, RequestConfig, InferStats
...
def _prepare(infer_backend: Literal['vllm', 'pt', 'lmdeploy']):
    ...
    infer_requests = [
        # InferRequest([{'role': 'user', 'content': '晚上睡不着觉怎么办'}]) for i in range(1)
        InferRequest([{'role': 'user', 'content': 'hello! who are you'}]) for i in range(1)
    ]
